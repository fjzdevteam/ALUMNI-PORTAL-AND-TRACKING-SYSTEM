<x-app-layout :title="'Analytics'" :navType="'admin'">
    <x-slot name="header">
        <div class="flex flex-row gap-4">
            ...
            <div>
                ...
                <p class="text-gray-600 text-base mt-1">Alignment of first jobs with degree field</p>
            </div>
        </div>
    </x-slot>
    ...
    <div class="grid grid-cols-2 gap-5 mb-6">
        <div class="flex flex-col">
            ...
            <form method="GET" action="{{ route('job-relevance') }}">
    ...
    <x-white-card class="p-6 mb-7">
        ...
        <p>Export job relevance analytics</p>
        <div class="flex space-x-4 mt-4">
            <x-export-button class="export-btn"
                href="{{ route('job.relevance.export.csv', [
    ...
    <x-white-card class="p-6 mb-5">
        <div class="w-full max-w-md mx-auto">
            <canvas id="jobChart" class="w-full h-64"></canvas>
            <form id="exportPdfForm" action="{{ route('job.relevance.export.pdf') }}" method="GET">
                <input type="hidden" name="college" value="{{ request('college') }}">
                <input type="hidden" name="program" value="{{ request('program') }}">
...
<script>
    const chartData = @json($jobRelevanceChartData);

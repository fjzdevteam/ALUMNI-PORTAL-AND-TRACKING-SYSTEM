<x-app-layout :title="'Analytics'" :navType="'admin'">
        ...
        <div class="flex flex-row gap-4">
            ...
            <div>
                <h2 class="font-bold text-3xl sm:text-4xl text-gray-800 leading-tight flex items-center gap-2">
                    {{ __('Alumni Engagement') }}
                </h2>
                <p class="text-gray-600 text-base mt-1">Participation of alumni in events (RSVP records)</p>
            </div>
        </div>
    </x-slot>
    ...
    <div class="grid grid-cols-2 gap-5 mb-6">
        <div class="flex flex-col">
            ...
            <form method="GET" action="{{ route('alumni-engagement') }}">
    ...
    <x-white-card class="p-6 mb-7">
        ...
        <p>Export alumni engagement analytics</p>
        <div class="flex space-x-4 mt-4">
            <x-export-button class="export-btn"
                href="{{ route('alumni.engagement.export.csv', [
    ...
    <x-white-card class="p-6 mb-5">
        <div class="w-full max-w-3xl mx-auto">
            ...
            <form id="exportPdfForm" action="{{ route('alumni.engagement.export.pdf') }}" method="GET">
                <input type="hidden" name="college" value="{{ request('college') }}">
                <input type="hidden" name="program" value="{{ request('program') }}">
...
<script>
    const chartLabels = ['Going', 'Maybe', 'Not Going'];
    const chartData = @json($engagementChartData);
    ...
    function exportPdfWithChart() {
        ...
        const base64Image = engagementChart.toBase64Image('image/png', 1.0);

<x-app-layout :title="'Analytics'" :navType="'admin'">
        ...
        <div class="flex flex-row gap-4">
            ...
            <div>
                <h2 class="font-bold text-3xl sm:text-4xl text-gray-800 leading-tight flex items-center gap-2">
                    {{ __('Unemployment Period') }}
                </h2>
                <p class="text-gray-600 text-base mt-1">
                    Time taken by alumni to secure their first job after graduation
                </p>
            </div>
        </div>
    </x-slot>
    ...
    <div class="grid grid-cols-2 gap-5 mb-6">
        <div class="flex flex-col">
            ...
            <form method="GET" action="{{ route('unemployment-period') }}">
    ...
    <x-white-card class="p-6 mb-7">
        ...
        <p>Export unemployment period analytics</p>
        <div class="flex space-x-4 mt-4">
            <x-export-button class="export-btn"
                href="{{ route('unemployment.period.export.csv', [
    ...
    <x-white-card class="p-6 mb-5">
        <div class="w-full max-w-2xl mx-auto">
            <canvas id="waitingChart" class="w-full h-64"></canvas>
            <form id="exportPdfForm" action="{{ route('unemployment.period.export.pdf') }}" method="GET">
                <input type="hidden" name="college" value="{{ request('college') }}">
                <input type="hidden" name="program" value="{{ request('program') }}">
...
<script>
    const chartData = @json($waitingChartData);
    ...
    function exportPdfWithChart() {
        ...
        const base64Image = waitingChart.toBase64Image('image/png', 1.0);

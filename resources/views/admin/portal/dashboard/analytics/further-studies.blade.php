<x-app-layout :title="'Analytics'" :navType="'admin'">
    <x-slot name="header">
        <div class="flex flex-row gap-4">
            ...
            <div>
                ...
                <p class="text-gray-600 text-base mt-1">Percentage of alumni pursuing graduate education</p>
            </div>
        </div>
    </x-slot>
    ...
    <div class="grid grid-cols-2 gap-5 mb-6">
        <div class="flex flex-col">
            ...
            <form method="GET" action="{{ route('further-studies') }}">
    ...
    <x-white-card class="p-6 mb-7">
        ...
        <p>Export further studies analytics</p>
        <div class="flex space-x-4 mt-4">
            <x-export-button class="export-btn"
                href="{{ route('further.studies.export.csv', [
    ...
    <x-white-card class="p-6 mb-5">
        <div class="w-full max-w-md mx-auto">
            <canvas id="furtherChart" class="w-full h-64"></canvas>
            <form id="exportPdfForm" action="{{ route('further.studies.export.pdf') }}" method="GET">
                <input type="hidden" name="college" value="{{ request('college') }}">
                <input type="hidden" name="program" value="{{ request('program') }}">
...
<script>
    const chartData = @json($furtherStudyChartData);
    const total = chartData.reduce((a, b) => a + b, 0);
    const canvas = document.getElementById('furtherChart');
    let studyChart = null;

    if (total === 0) {
        const wrapper = canvas.parentElement;
        wrapper.innerHTML = `<div class="flex items-center justify-center h-48 text-md">No data available</div>`;
    } else {
    ...
    function exportPdfWithChart() {
        ...
        const base64Image = studyChart.toBase64Image('image/png', 1.0);
